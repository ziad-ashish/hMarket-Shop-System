...
const InventoryEntry = (() => {
  ...
  async function open() {
    ...
    Modal.open({title:'إضافة فاتورة مشتريات',size:'lg',body:`
      ...
      <div class="form-row"><label>وحدة الشراء<input class="form-control" id="entryPurchaseUnit" list="entryUnitsList" maxlength="40" placeholder="اختر أو اكتب وحدة جديدة" value="قطعة"></label><label>وحدة البيع<input class="form-control" id="entrySaleUnit" list="entryUnitsList" maxlength="40" placeholder="اختر أو اكتب وحدة جديدة" value="قطعة"></label><datalist id="entryUnitsList">${['قطعة','جهاز','طقم','بكرة','لفة','كرتونة','علبة','شريط','كيس','متر','كيلو','جرام','لتر','مللي','خدمة'].map(u=>`<option value="${u}"></option>`).join('')}</datalist><label>عدد وحدات البيع في العبوة<input id="entryFactor" class="form-control" type="number" min="0.001" step="any" value="1"></label></div>
      <div class="form-row"><label>كمية الشراء<input id="entryQty" class="form-control" type="number" min="0.001" step="any" value="1"></label><label>تكلفة وحدة الشراء<input id="entryCost" class="form-control" type="number" min="0" step="0.01" value="0"></label><label>سعر القطاعي<input id="entryPrice" class="form-control" type="number" min="0" step="0.01" value="0"></label><label>سعر الجملة (اختياري)<input id="entryWholesalePrice" class="form-control" type="number" min="0" step="0.01" placeholder="بدون سعر جملة"></label><label>أقل كمية لسعر الجملة<input id="entryWholesaleMinQty" class="form-control" type="number" min="1" step="1" value="1"></label><label>عدد ملصقات الباركود (اختياري)<input id="entryCopies" class="form-control" type="number" min="0" max="200" step="1" value="0"></label></div>
    ...
    const configure=()=>{const p=products.find(p=>p.id===el('Product').value);el('NewFields').hidden=!!p;['PurchaseUnit','SaleUnit','Factor','Divisible'].forEach(k=>el(k).disabled=!!p);if(p){el('PurchaseUnit').value=p.purchaseUnit;el('SaleUnit').value=p.saleUnit;el('Factor').value=p.conversionFactor||1;el('Cost').value=p.cost*(p.conversionFactor||1);el('Price').value=p.price;el('WholesalePrice').value=p.wholesalePrice??'';el('WholesaleMinQty').value=p.wholesaleMinQty||1;}el('SerialLabel').hidden=!p?.trackSerial;summary();};
    ...
    el('Save').onclick=async()=>{
      ...
      try {
        ...
        const print=()=>{if(DeviceSettings.get().labelPrinter)window.open(`/api/print_labels?product_ids=${encodeURIComponent(result.product_id)}&copies=${result.copies}`,'_blank');else Toast.warn('طابعة الملصقات موقوفة','فعّلها من الإعدادات أو اطبع لاحقًا');};
